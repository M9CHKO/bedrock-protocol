...
// ----------------------------------------------------------------------------------------------------------------------------
void CCRakNetUDT::OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond BValue, BytesPerMicrosecond ASValue, double totalUserDataBytesAcked, bool isContinuousSend, DatagramSequenceNumberType sequenceNumber )
{
#if CC_TIME_TYPE_BYTES==4
	RakAssert(rtt < 10000);
#else
	...
	if (hasBAndAS)
	{
		///	RakAssert(BValue!=UNDEFINED_TRANSFER_RATE && ASValue!=UNDEFINED_TRANSFER_RATE);
		//	B=B * .875 + BValue * .125;
		// AS is packet arrival rate
		RakAssert(ASValue!=UNDEFINED_TRANSFER_RATE);
		AS=ASValue;
...
// ----------------------------------------------------------------------------------------------------------------------------
void CCRakNetUDT::OnSendAckGetBAndAS(CCTimeType curTime, bool *hasBAndAS, BytesPerMicrosecond *BValue, BytesPerMicrosecond *ASValue)
{
	if (curTime>lastTransmitOfBAndAS+SYN)
	{
		*BValue=0;
		*ASValue=ReceiverCalculateDataArrivalRate(curTime);

		if (*ASValue==UNDEFINED_TRANSFER_RATE)
		{
			*hasBAndAS=false;
		}
		else
